...
def main():
    ...
    skybox = pg.image.load("obj models/blue.jpeg").convert()
    skybox = pg.transform.smoothscale(
        skybox, (SCREEN_W * (np.pi * 2 / FOV_H + 1), SCREEN_H * (np.pi / FOV_V + 1))
    )
    skybox = pg.surfarray.array3d(skybox)

    model4 = Model("obj models/cube text.obj", "obj models/white.png")
    model4.change_position(y=15)

    model8 = Model("obj models/sphere.obj", "obj models/white.png")
    model8.change_position(y=8)

    camera = np.asarray([11.0, 14.5, 68, 4.4, 0])
    light_camera = np.asarray([-500000.0, 1000000.0, -500000.0, 0.8, 1.0])  # /10
    ...
    while running:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                if event.key == pg.K_DELETE:
                    camera = np.asarray([11.0, 14.5, 68, 4.4, 0])  # reset camera

        # frame[:,:,:] = SKY_BLUE
        h1 = int(SCREEN_W * camera[3] / FOV_H)
        v1 = int(SCREEN_H * camera[4] / FOV_V + 2 * SCREEN_H)
        frame[:, :, :] = skybox[
            h1 : h1 + SCREEN_W, v1 : v1 + SCREEN_H, :
        ]  # crude cylindrical skybox
        ...
        pg.display.set_caption(
            str(round(1 / (elapsed_time + 1e-32), 1)) + " " + str(np.round(camera, 1))
        )
...
def movement(camera, elapsed_time):
    if pg.mouse.get_focused():
        ...
        camera[3] = (
            ...
            + 10
            * elapsed_time
            * np.clip((p_mouse[0] - SCREEN_W / 2) / SCREEN_W, -0.2, 0.2)
        ) % (2 * np.pi)
        camera[4] = camera[4] + 10 * elapsed_time * np.clip(
            (p_mouse[1] - SCREEN_H / 2) / SCREEN_H, -0.2, 0.2
        )
    ...
    if (pressed_keys[ord("w")] or pressed_keys[ord("s")]) and (
        pressed_keys[ord("a")] or pressed_keys[ord("d")]
    ):
...
class Model:
    ...
    def __init__(self, path_obj, path_texture=""):
        ...
        if self.textured and path_texture != "":
            ...
            self.texture_uv, self.texture_map = np.ones((2, 2)), np.random.randint(
                1, 2, (2, 3)
            )
    ...
    def change_position(self, x=0, y=0, z=0, rotx=0, roty=0, rotz=0, scale=1, reset=0):
        if reset:
            ...
            self.position = self.position + np.asarray(
        ...
        if self.position[3] != 0:
            ...
            self.points[:, 1] = temp_points[:, 1] * np.cos(
                self.position[3]
            ) - temp_points[:, 2] * np.sin(self.position[3])
            self.points[:, 2] = temp_points[:, 1] * np.sin(
                self.position[3]
            ) + temp_points[:, 2] * np.cos(self.position[3])

        # rotate around y axis
        if self.position[4] != 0:
            temp_points = self.points.copy()
            self.points[:, 0] = temp_points[:, 0] * np.cos(
                self.position[4]
            ) - temp_points[:, 2] * np.sin(self.position[4])
            self.points[:, 2] = temp_points[:, 0] * np.sin(
                self.position[4]
            ) + temp_points[:, 2] * np.cos(self.position[4])
        ...
        if self.position[5] != 0:
            ...
            self.points[:, 0] = temp_points[:, 0] * np.cos(
                self.position[5]
            ) - temp_points[:, 1] * np.sin(self.position[5])
            self.points[:, 1] = temp_points[:, 0] * np.sin(
                self.position[5]
            ) + temp_points[:, 1] * np.cos(self.position[5])
...
def render_frame(camera, frame, z_buffer, shadow_map, light_camera):
    ...
    h_vector = np.asarray([light_vector[0], light_vector[2]]) / np.sqrt(
        light_vector[0] * light_vector[0] + light_vector[2] * light_vector[2]
    )
    light_camera[3] = np.arccos(dot_product(h_vector, np.asarray([1, 0])))
    light_camera[4] = np.arcsin(
        dot_product(light_vector / lenght, np.asarray([0, -1, 0]))
    )

    if np.sign(np.sin(light_camera[3])) != np.sign(
        light_vz
    ):  # not sure, sometimes points in the wrong direction
...
def render_shadow_map(points, triangles, light_camera, shadow_map):
    for index in range(len(triangles)):
        ...
        CameraRay = (points[triangle[0]][:3] - light_camera[:3]) / points[triangle[0]][
            5
        ]
        ...
        z_min = min(
            [points[triangle[0]][5], points[triangle[1]][5], points[triangle[2]][5]]
        )
        ...
        if filter_triangles(z_min, normal, -CameraRay, xxs, yys):
            ...
            x_slopes = get_slopes(
                start[0], middle[0], stop[0], start[1], middle[1], stop[1]
            )
            z_slopes = get_slopes(
                start[2], middle[2], stop[2], start[1], middle[1], stop[1]
            )
            ...
            for y in range(max(1, int(start[1])), min(SCREEN_H - 1, int(stop[1] + 1))):
                ...
                xx1, xx2 = max(1, min(SCREEN_W - 1, int(x1))), max(
                    1, min(SCREEN_W - 1, int(x2 + 1))
                )
                if xx1 != xx2:
                    z_slope = (z2 - z1) / (x2 - x1 + 1e-32)
                    if (
                        min(shadow_map[xx1:xx2, y]) == 1e32
                    ):  # check z buffer, fresh pixels
                        shadow_map[xx1:xx2, y] = (
                            np.arange(xx1, xx2) - x1
                        ) * z_slope + z1
                    else:
...
def project_points(points, camera, shadow_mod=1):
    ...
    points[:, 3] = (
        -hor_fov_adjust * points2[:, 3] / points[:, 5] + 0.5 * SCREEN_W
    ).astype(np.int32)
    points[:, 4] = (
        -ver_fov_adjust * points[:, 4] / points[:, 5] + 0.5 * SCREEN_H
    ).astype(np.int32)
...
def draw_model(
    ...
    for index in range(len(triangles)):
        ...
        # check valid values
        if filter_triangles(z_min, normal, CameraRay, xxs, yys):
            lightRay = (points[triangle[0]][:3] - light_camera[:3]) / points[
                triangle[0]
            ][5]
            lightRay = lightRay / np.sqrt(
                lightRay[0] * lightRay[0]
                + lightRay[1] * lightRay[1]
                + lightRay[2] * lightRay[2]
            )
            shade1 = 0.2 + 0.8 * (
                -0.5 * dot_product(lightRay, normal) + 0.5
            ...
            if textured:
                ...
                color0 = (
                    np.abs(points[triangles[index][0]][:3]) * color_scale + 25
                ) * z0
                color1 = (
                    np.abs(points[triangles[index][1]][:3]) * color_scale + 25
                ) * z1
                color2 = (
                    np.abs(points[triangles[index][2]][:3]) * color_scale + 25
                ) * z2

            # barycentric denominator, based on https://codeplea.com/triangular-interpolation
            denominator = 1 / (
                (proj_points[1][1] - proj_points[2][1])
                * (proj_points[0][0] - proj_points[2][0])
                + (proj_points[2][0] - proj_points[1][0])
                * (proj_points[0][1] - proj_points[2][1])
                + 1e-32
            )
            ...
            for y in range(max(0, int(start[1])), min(SCREEN_H, int(stop[1] + 1))):
                ...
                minx, maxx = max(0, min(x1, x2, SCREEN_W)), min(
                    SCREEN_W, max(0, x1 + 1, x2 + 1)
                )
                ...
                for x in range(int(minx), int(maxx)):
                    ...
                    w1 = (
                        (proj_points[2][1] - proj_points[0][1])
                        * (x - proj_points[2][0])
                        + (proj_points[0][0] - proj_points[2][0])
                        * (y - proj_points[2][1])
                    ) * denominator
                    ...
                    if z < z_buffer[x, y] and z >= min_z:
                        ...
                        if shade1 < 0.6:
                            ...
                            lx, ly = max(0, min(SCREEN_W - 1, int(point[0]))), max(
                                0, min(SCREEN_H - 1, int(point[1]))
                            )
...
def get_slopes(num_start, num_middle, num_stop, den_start, den_middle, den_stop):
    slope_1 = (num_stop - num_start) / (
        den_stop - den_start + 1e-32
    )  # + 1e-32 avoid zero division ¯\_(ツ)_/¯
...
def filter_triangles(
    z_min, normal, CameraRay, xxs, yys, scale=1
):  # TODO replace filtering with proper clipping
    # only points on +z, facing the camera, check triangle bounding box
    if (
        z_min > 0
        and dot_product(normal, CameraRay) < 0
        and max(xxs) >= 0
        and min(xxs) < SCREEN_W * scale
        and max(yys) >= 0
        and min(yys) < SCREEN_H * scale
    ):
        return True
    else:
        return False
